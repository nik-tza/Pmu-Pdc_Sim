...
public abstract class TaskAbstract implements Task, Comparable<Task> {
	...
	protected Map<Integer, Double> hopDelays = new HashMap<>();
	...
	public void setArrivalTime(double clock) {
		...
		// Don't set execStartTime here - it should be set only when execution actually starts
	}
	...
	public void addHopDelay(int hopNumber, double delay) {
		// Store the delay for this hop (replace, don't add to existing value)
		hopDelays.put(hopNumber, delay);
	}

	public double getHopDelay(int hopNumber) {
		return hopDelays.getOrDefault(hopNumber, 0.0);
	}

	public Map<Integer, Double> getAllHopDelays() {
		return new HashMap<>(hopDelays);
	}
}
