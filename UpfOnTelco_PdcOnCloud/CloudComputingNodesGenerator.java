...
/**
 * Cloud Computing Nodes Generator for Smart Grid simulation.
 * Extends DefaultComputingNodesGenerator to read from XML files
 * and applies Cloud-specific configurations for sensor devices.
 */
public class CloudComputingNodesGenerator extends DefaultComputingNodesGenerator {

    private static Long cachedPmuPlacementSeed = null;
    private static int pmuCounter = 0;
    
    public CloudComputingNodesGenerator(SimulationManager simulationManager,
    ...
    private long getPmuPlacementSeed() {
        ...
        try (FileInputStream input = new FileInputStream("UpfOnTelco_PdcOnCloud/settings/simulation_parameters.properties")) {
            ...
            getSimulationManager().getSimulationLogger().deepLog(
                "CloudComputingNodesGenerator - Loaded PMU placement seed: " + cachedPmuPlacementSeed);
                
            return cachedPmuPlacementSeed;
        } catch (IOException | NumberFormatException e) {
            getSimulationManager().getSimulationLogger().print(
                "CloudComputingNodesGenerator - Warning: Could not read pmu_placement_seed, using default 12345");
    ...
    protected void insertEdgeDevice(ComputingNode newDevice) {
        ...
        // Apply Cloud-specific configurations for sensor devices
        configureCloudDevice(newDevice);
    }
    
    /**
     * Applies Cloud-specific configurations to devices.
     */
    private void configureCloudDevice(ComputingNode device) {
        if (device.isGeneratingTasks()) {
            // Configure as Cloud sensor device
            device.setAsSensor(true);
            device.enableTaskGeneration(true);
            
            getSimulationManager().getSimulationLogger().print(
                "CloudComputingNodesGenerator - Configured Cloud sensor device " + 
    ...
    protected ComputingNode createComputingNode(Element datacenterElement, SimulationParameters.TYPES type)
        ...
        if (type == SimulationParameters.TYPES.EDGE_DEVICE) {
            return createSeededEdgeDevice(datacenterElement, type);
        } else {
    ...
    /**
     * **NEW: Create Edge device with seeded random placement**
     */
    private ComputingNode createSeededEdgeDevice(Element datacenterElement, SimulationParameters.TYPES type)
            throws NoSuchAlgorithmException, NoSuchMethodException, SecurityException, InstantiationException,
            IllegalAccessException, IllegalArgumentException, InvocationTargetException {
        ...
        if (pmuSeed != -1) {
            ...
            getSimulationManager().getSimulationLogger().deepLog(
                "CloudComputingNodesGenerator - Using seeded random for PMU " + pmuCounter + 
            ...
            getSimulationManager().getSimulationLogger().deepLog(
                "CloudComputingNodesGenerator - Using secure random for PMU " + pmuCounter);
            pmuCounter++;
        }
        ...
        getSimulationManager().getSimulationLogger().deepLog(
            "CloudComputingNodesGenerator - PMU device " + getMistOnlyList().size() + 
    ...
    private Location generateSmartPmuLocation(Random random) {
        ...
        getSimulationManager().getSimulationLogger().deepLog(
            "CloudComputingNodesGenerator - PMU " + currentPmuIndex + 
            " placed at (" + String.format("%.1f", x) + "," + String.format("%.1f", y) + ")" +
            " in shuffled cell [" + gridRow + "," + gridCol + "] of [" + gridRows + "x" + gridCols + "]"
        );
    ...
    /**
     * **NEW: Helper methods to access protected fields from parent class**
     */
