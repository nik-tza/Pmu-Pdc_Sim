...
public class CloudTaskGenerator extends DefaultTaskGenerator {
    ...
    public CloudTaskGenerator(SimulationManager simulationManager) {
        ...
        System.out.println("CloudTaskGenerator - Initialized for PMU data transfers:");
    ...
    private int readPmuRateFromApplications() {
        try {
            ...
            if (!SimulationParameters.applicationList.isEmpty()) {
                ...
                System.out.printf("CloudSimulationManager - DEBUG: Read rate from applications.xml: %d tasks/second%n", ratePerMinute);
                return ratePerMinute;
            } else {
                System.err.println("CloudSimulationManager - ERROR: applicationList is empty!");
            }
        } catch (Exception e) {
            System.err.println("CloudSimulationManager - Error reading rate from applications.xml: " + e.getMessage());
            e.printStackTrace();
        }
        
        // Fallback to 1 measurement per second
        System.out.println("CloudSimulationManager - WARNING: Using fallback rate: 1 measurement/second");
        return 1;
    }
    ...
    public FutureQueue<Task> generate() {
        ...
        for (int second = 0; second < totalSeconds; second++) {
            ...
            for (int measurement = 0; measurement < TASKS_PER_SECOND; measurement++) {
                ...
                if (second == 0 && measurement < 3) {
                    System.out.printf("CloudSimulationManager - DEBUG: Creating tasks for time %.3f (second=%d, measurement=%d/%d)%n", 
        ...
        System.out.printf("CloudSimulationManager - Generated %d synchronized PMU data tasks for %d PMUs over %d seconds at %d measurements/second%n",
        ...
        System.out.printf("CloudSimulationManager - Expected: %d tasks, Actual: %d tasks, Match: %s%n",
