...
public class CloudDataCollectorDynamic extends SimEntity {
    ...
    public CloudDataCollectorDynamic(SimulationManager simulationManager, ComputingNode tsoNode) {
        ...
        System.out.printf("CloudDataCollectorDynamic - Initialized for %d PMUs (distance delays handled by CloudNetworkModel)%n", 
                         REQUIRED_PMU_COUNT);
    }
        ...
        synchronized(this) {
            switch (ev.getTag()) {
                ...
                case GENERATION_TIME_TIMEOUT:
                    ...
                    break;
                default:
                    System.err.println("CloudDataCollectorDynamic - Unknown event: " + ev.getTag());
            }
        }
    }
    ...
    public void collectPmuData(Task dataTask) {
        ...
        // **Get network transfer details from CloudNetworkModel (consistent with actual transfer)**
        CloudNetworkModel networkModel = getNetworkModel();
        CloudNetworkModel.NetworkTransferResult transferResult = null;
    ...
    private void handleGenerationTimeTimeout(int batchId) {
        ...
        System.out.printf("CloudDataCollectorDynamic - Processing Generation Time %.3f with %d tasks%n", 
        ...
        for (TaskWithArrivalTime taskWithTime : tasks) {
            if (taskWithTime.realArrivalTime <= deadline) {
                ...
                CloudLogger.getInstance().logPmuDataTransferFull(
                    taskWithTime.task, taskWithTime.pmuId, PMU_DATA_SIZE_KB, 
                    taskWithTime.path, false, taskWithTime.networkDelay);
                
                System.out.printf("CloudDataCollectorDynamic - ON-TIME: PMU %d at %.4f (deadline: %.4f, margin: %.4fs)%n", 
                ...
                System.out.printf("CloudDataCollectorDynamic - LATE ARRIVAL: PMU %d at %.4f (deadline: %.4f, late by: %.4fs) - REJECTED%n", 
                                 taskWithTime.pmuId, taskWithTime.realArrivalTime, deadline, 
                                 taskWithTime.realArrivalTime - deadline);
            }
        }
        
        System.out.printf("CloudDataCollectorDynamic - Collection Window Summary: %d on-time, %d dropped (%.1f%% success)%n", 
    ...
    private void createStateEstimationTaskFromBuffer(double generationTime, List<TaskWithArrivalTime> onTimeTasks, 
        ...
        if (onTimeTasks.isEmpty()) {
            System.out.printf("CloudDataCollectorDynamic - No on-time measurements for Generation Time %.3f%n", generationTime);
            return;
        }
        ...
        if (onTimeTasks.size() >= REQUIRED_PMU_COUNT) {
            ...
            if (arrivalTimes.size() > 1) {
                ...
                System.out.printf("CloudDataCollectorDynamic - ALL PMU ON TIME: PDC Waiting Time: %.4fs (from %.4fs to %.4fs), Finished early!%n", 
            ...
            System.out.printf("CloudDataCollectorDynamic - INCOMPLETE ARRIVAL: PDC Waiting Time: %.4fs (waited full timeout), Got %d/%d PMUs%n", 
        ...
        try {
            ...
            CloudLogger.getInstance().logStateEstimationTaskCreation(
            ...
            System.out.printf("CloudDataCollectorDynamic - Created Grid Analysis Task %d for Generation Time %.3f (%d/%d PMUs, %.4fs PDC wait, %.2f KB data)%n",
            ...
            System.err.println("CloudDataCollectorDynamic - Error creating grid analysis task: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private int extractPmuId(ComputingNode edgeDevice, Task dataTask) {
        if (edgeDevice != null) {
            ...
            try {
                ...
                System.err.println("CloudDataCollectorDynamic - Error finding PMU ID from device list: " + e.getMessage());
            }
        }
    ...
    private int extractPmuIdFromTask(Task dataTask) {
        try {
            ...
            System.err.println("CloudDataCollectorDynamic - Error extracting PMU ID from task: " + e.getMessage());
            return 0; // Default fallback
        }
    }
    
    /**
     * Get the CloudNetworkModel instance from simulation manager
     */
    private CloudNetworkModel getNetworkModel() {
        try {
            // Get the network model from simulation manager
            if (simulationManager.getNetworkModel() instanceof CloudNetworkModel) {
                return (CloudNetworkModel) simulationManager.getNetworkModel();
            }
        } catch (Exception e) {
            System.err.println("CloudDataCollectorDynamic - Could not get CloudNetworkModel: " + e.getMessage());
        }
        return null;
    }
