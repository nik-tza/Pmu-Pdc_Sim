...
/**
 * Edge Computing Nodes Generator for Smart Grid simulation.
 * Extends DefaultComputingNodesGenerator to read from XML files
 * and applies Edge-specific configurations for sensor devices.
 */
public class EdgeComputingNodesGenerator extends DefaultComputingNodesGenerator {

    private static Long cachedPmuPlacementSeed = null;
    private static int pmuCounter = 0;
    
    public EdgeComputingNodesGenerator(SimulationManager simulationManager,
    ...
    private long getPmuPlacementSeed() {
        ...
        try (FileInputStream input = new FileInputStream("UpfOnEdge_PdcOnEdge/settings/simulation_parameters.properties")) {
            ...
            getSimulationManager().getSimulationLogger().deepLog(
                "EdgeComputingNodesGenerator - Loaded PMU placement seed: " + cachedPmuPlacementSeed);
                
            return cachedPmuPlacementSeed;
        } catch (IOException | NumberFormatException e) {
            getSimulationManager().getSimulationLogger().print(
                "EdgeComputingNodesGenerator - Warning: Could not read pmu_placement_seed, using default 12345");
    ...
    protected void insertEdgeDevice(ComputingNode newDevice) {
        ...
        // Apply Edge-specific configurations for sensor devices
        configureEdgeDevice(newDevice);
    }
    
    /**
     * Applies Edge-specific configurations to devices.
     */
    private void configureEdgeDevice(ComputingNode device) {
        if (device.isGeneratingTasks()) {
            // Configure as Edge sensor device
            device.setAsSensor(true);
            device.enableTaskGeneration(true);
            
            getSimulationManager().getSimulationLogger().print(
                "EdgeComputingNodesGenerator - Configured Edge sensor device " + 
    ...
    private ComputingNode createSeededEdgeDevice(Element datacenterElement, SimulationParameters.TYPES type)
        ...
        if (pmuSeed != -1) {
            ...
            getSimulationManager().getSimulationLogger().deepLog(
                "EdgeComputingNodesGenerator - Using seeded random for PMU " + pmuCounter + 
            ...
            getSimulationManager().getSimulationLogger().deepLog(
                "EdgeComputingNodesGenerator - Using secure random for PMU " + pmuCounter);
            pmuCounter++;
        }
        ...
        getSimulationManager().getSimulationLogger().deepLog(
            "EdgeComputingNodesGenerator - PMU device " + getMistOnlyList().size() + 
    ...
    private Location generateSmartPmuLocation(Random random) {
        ...
        getSimulationManager().getSimulationLogger().deepLog(
            "EdgeComputingNodesGenerator - PMU " + currentPmuIndex + 
