 ...
 */
public class EdgeNetworkLink extends NetworkLink {
    
    public EdgeNetworkLink(ComputingNode src, ComputingNode dest, SimulationManager simulationManager, NetworkLinkTypes type) {
    ...
    protected void transferFinished(TransferProgress transfer) {
        ...
        if (transfer.getTransferType() == TransferProgress.Type.REQUEST) {
            ...
            EdgeSimulationManager edgeManager = (EdgeSimulationManager) simulationManager;
                ...
                EdgeDataCollectorDynamic pmuCollector = edgeManager.getDataCollectorForPmu(transfer.getTask().getEdgeDevice());
                if (pmuCollector != null) {
                    // Forward to the correct GNB collector
                    scheduleNow(pmuCollector, EdgeDataCollectorDynamic.EDGE_DATA_RECEIVED, transfer.getTask());
    ...
    private void handlePmuDataReceived(TransferProgress transfer) {
        ...
        if (simulationManager instanceof EdgeSimulationManager) {
            EdgeSimulationManager edgeSimManager = (EdgeSimulationManager) simulationManager;
            
            // Find the appropriate GNB collector for this PMU data
            EdgeDataCollectorDynamic pmuCollector = edgeSimManager.getDataCollectorForPmu(transfer.getTask().getEdgeDevice());
            if (pmuCollector != null) {
                // Forward to the correct GNB collector
                scheduleNow(pmuCollector, EdgeDataCollectorDynamic.EDGE_DATA_RECEIVED, transfer.getTask());
            } else {
                // Fallback: schedule data collection event directly to simulation manager
                scheduleNow(simulationManager, 300, transfer.getTask()); // EDGE_DATA_RECEIVED = 300
            }
        } else {
