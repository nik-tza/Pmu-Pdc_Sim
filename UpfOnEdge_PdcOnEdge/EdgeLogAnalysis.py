...
# Configuration
BASE_OUTPUT_DIR = "UpfOnEdge_PdcOnEdge/output"
PMU_SETTINGS_DIR = "UpfOnEdge_PdcOnEdge/settings"
SHOW_PLOTS = True
...
def create_simulation_map(simulation_folder: str, logger: logging.Logger):
    ...
    # **Removed network connections - distributed architecture with direct PMU->GNB only**
    
    # Plot settings
    plt.grid(True, linestyle='--', alpha=0.3)
    plt.xlabel('X Coordinate (meters)', fontsize=12)
    plt.ylabel('Y Coordinate (meters)', fontsize=12)
    plt.title(f'PMU Smart Grid Simulation Map\n{max_pmus} PMU Sensors and {len(datacenters)} GNB Edge Datacenters (Distributed Architecture)', 
    ...
    legend_elements = [
        ...
        plt.Line2D([0], [0], color='gray', linestyle='--', alpha=0.5, label='PMU-GNB Direct Links'),
...
"""
    ...
    pmu_csv = None
    for file in os.listdir(simulation_folder):
        if file.endswith("_pmu.csv") or file == "Sequential_simulation_pmu.csv":
...
DETAILED TRANSFERS PER PMU:"""
                ...
                if pmu_csv and os.path.exists(pmu_csv):
                    ...
                    for pmu_id in df_detailed['PmuID'].unique():
                        ...
                        ok_transfers = len(pmu_rows[pmu_rows['Status'] == 'OK'])
