 ...
 */
public class UpfTelcoTaskOrchestrator extends DefaultOrchestrator {
    
    // Data collection configuration
    private final int TOTAL_PMU_COUNT;
    // NOTE: Data collectors are now managed by UpfTelcoSimulationManager (distributed per-GNB)
    
    // Statistics
    private int totalDataTransfers = 0;
    private int totalStateEstimationTasks = 0;
    
    public UpfTelcoTaskOrchestrator(SimulationManager simulationManager) {
        ...
        System.out.println("PmuTaskOrchestrator - Initialized for DISTRIBUTED GNB collection with 3-hop path:");
        System.out.println("  - Network Path: PMU → closest GNB → TELCO → GNB (collection window starts at final GNB)");
        System.out.println("  - Dynamic window PMU measurement collection per GNB after round trip through TELCO");
    ...
    public void initialize() {
        ...
        // Data collectors are initialized by UpfTelcoSimulationManager during simulation start
        // No need to initialize them here anymore
        
        // Log initialization
        int gnbCount = getGnbCount();
        simulationManager.getSimulationLogger().print("PmuTaskOrchestrator - Initialized for DISTRIBUTED GNB collection with 3-hop path:");
        simulationManager.getSimulationLogger().print("  - Network Path: PMU → GNB → TELCO → GNB (collection starts after round trip)");
        simulationManager.getSimulationLogger().print("  - Dynamic window (0.045s from first arrival at GNB after TELCO round trip)");
    ...
    public synchronized void orchestrate(Task task) {
        ...
        if (simulationManager instanceof UpfTelcoSimulationManager) {
            ...
            UpfTelcoDataCollectorDynamic collector = upfTelcoManager.getDataCollectorForPmu(sourceDevice);
            
            if (collector != null) {
                // Forward the genuine PMU measurement data to the appropriate GNB collector
                collector.collectUpfTelcoData(task);
            ...
            simulationManager.getSimulationLogger().print("PmuTaskOrchestrator - ERROR: SimulationManager is not UpfTelcoSimulationManager");
        }
    }
    ...
    public void resultsReturned(Task task) {
        ...
        if (isGridAnalysisTask) {
            ...
            UpfTelcoLogger.getInstance().logGridAnalysisTaskCompletion(task, success);
    ...
    public String getOrchestratorStatus() {
        if (simulationManager instanceof UpfTelcoSimulationManager) {
            ...
            return String.format("UpfTelco Distributed GNB Orchestrator: %d data transfers, %d grid analysis tasks - Distributed collectors (%d GNBs)",
                               totalDataTransfers, totalStateEstimationTasks, gnbCount);
        }
        return "UpfTelco Distributed GNB Orchestrator: No simulation manager found";
    }
