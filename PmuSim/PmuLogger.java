...
public class PmuLogger {
    private static PmuLogger instance = null;
    ...
    private PmuLogger(String path) throws IOException {
    ...
    public static PmuLogger initialize(SimulationManager simulationManager, String outputPath) throws IOException {
        if (instance == null) {
            instance = new PmuLogger(outputPath);
        }
    ...
    public static PmuLogger initialize(String outputPath) throws IOException {
        if (instance == null) {
            instance = new PmuLogger(outputPath);
        }
        return instance;
    ...
    public static PmuLogger getInstance() {
        if (instance == null) {
            throw new IllegalStateException("PmuLogger has not been initialized. Call initialize() first.");
        }
        return instance;
    }
    ...
    public void saveAllLogs() {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(getCSVFileName()))) {
            ...
            System.out.println("PmuLogger - Saved " + csvRecords.size() + " PMU data transfer records to CSV");
        } catch (IOException e) {
            System.out.println("PmuLogger - Error saving PMU data CSV: " + e.getMessage());
            e.printStackTrace();
        }
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(getStateEstimationCSVFileName()))) {
            ...
            System.out.println("PmuLogger - Saved " + stateEstimationCsvRecords.size() + " State Estimation task records to CSV");
        } catch (IOException e) {
            System.out.println("PmuLogger - Error saving State Estimation CSV: " + e.getMessage());
            e.printStackTrace();
        }
    ...
    /**
     * Log PMU network transfer using NetworkTransferResult from PmuNetworkModel
     */
    public void logPmuNetworkTransfer(com.mechalikh.pureedgesim.taskgenerator.Task dataTask, 
                                    PmuSim.PmuNetworkModel.NetworkTransferResult transferResult) {
        try {
            ...
            System.err.println("PmuLogger - Error logging network transfer: " + e.getMessage());
        }
    }
