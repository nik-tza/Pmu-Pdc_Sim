 ...
 */
public class PmuNetworkLink extends NetworkLink {
    
    public PmuNetworkLink(ComputingNode src, ComputingNode dest, SimulationManager simulationManager, NetworkLinkTypes type) {
    ...
    protected void transferFinished(TransferProgress transfer) {
        ...
        if (transfer.getTransferType() == TransferProgress.Type.REQUEST) {
            ...
            if (simulationManager instanceof PmuSimulationManager) {
                PmuSimulationManager pmuManager = (PmuSimulationManager) simulationManager;
                if (pmuManager.getDataCollector() != null) {
                    // Forward to data collector
                    scheduleNow(pmuManager.getDataCollector(), PmuDataCollectorDynamic.PMU_DATA_RECEIVED, transfer.getTask());
                    System.out.printf("PmuNetworkLink - Forwarded PMU data to collector: Task %d%n", transfer.getTask().getId());
                }
            }
        } else {
    ...
    private void calculatePmuDataHopDelays(TransferProgress transfer) {
        ...
        System.out.printf("PmuNetworkLink - Data transfer delays: PMU_%d → Hop1: %.4fs, Hop2: %.4fs, Hop3: %.4fs (Total: %.4fs)%n",
    ...
    private void handlePmuDataReceived(TransferProgress transfer) {
        System.out.printf("PmuNetworkLink - PMU measurement data received by TSO from %s at time %.4f%n",
        ...
        if (simulationManager instanceof PmuSimulationManager) {
            PmuSimulationManager pmuSimManager = (PmuSimulationManager) simulationManager;
            
            // Schedule the PMU data received event directly to the simulation manager
            scheduleNow(pmuSimManager, 300, transfer.getTask()); // PMU_DATA_RECEIVED = 300
        } else {
